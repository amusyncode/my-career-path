...
export async function POST(request: NextRequest) {
  try {
    ...
    // 학생 데이터 조회
    const [profileRes, goalsRes, skillsRes, projectsRes, certsRes, logsRes] = await Promise.all([
      supabase.from("profiles").select("*").eq("id", user_id).single(),
      supabase.from("roadmap_goals").select("title, category, status").eq("user_id", user_id),
      supabase.from("skills").select("name, level").eq("user_id", user_id),
      supabase.from("projects").select("title, status").eq("user_id", user_id),
      supabase.from("certificates").select("name").eq("user_id", user_id),
      supabase
        .from("daily_logs")
        .select("log_date, daily_goal, study_hours")
        .eq("user_id", user_id)
